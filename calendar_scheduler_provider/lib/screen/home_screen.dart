...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<ScheduleProvider>(); //프로바이더 변경이 있을 때마다 build() 함수 재실행. provider 패키지에 해당

    final selectedDate = provider.selectedDate;//선택된 날짜 가져오기

    final schedules = provider.cache[selectedDate] ?? [];//선택된 날짜의 일정 가져오기
